        ...
        async search() {
            ...
            if (this.options.guildId === "@me") {
                if (!this.options.channelId) {
                    log.error("Channel ID is required for DM messages!");
                    return { messages: [] };
                }
            ...
            try {
                ...
                const queryParams = [
                    ...
                    ["channel_id", this.options.channelId || undefined], // Simplified channel_id handling
                ...
                const validQueryParams = queryParams.filter(([_, value]) => value !== undefined);

                resp = await fetch(
                    API_SEARCH_URL + "search?" + queryString(validQueryParams),
                    {
                        headers: {
                            Authorization: this.options.authToken,
                        },
            ...
            let data;
            try {
                // Clone the response for potential reuse
                const respClone = resp.clone();
                data = await resp.json();

                // not indexed yet
                if (resp.status === 202) {
                    let w = data.retry_after;
                    w = !isNaN(w) ? w * 1000 : this.stats.searchDelay; // Fix retry_after 0
                    this.stats.throttledCount++;
                    this.stats.throttledTotalTime += w;
                    log.warn(
                ...
                if (!resp.ok) {
                    ...
                    if (resp.status === 400 && data.code === 50024) {
                        ...
                        const emptyData = { total_results: 0, messages: [] };
                        this.state._seachResponse = emptyData;
                        return emptyData;
                    }

                    if (resp.status === 403 && data.code === 50001) {
...
`;
    ...
    async function getChannelId() {
        ...
        if (m && m[1]) {
            ...
            try {
                ...
                if (!response.ok) {
                    log.warn(`Failed to verify channel ${channelId}: ${response.status}`);
                    return [channelId, false, null];
                }
                ...
                if ([10, 11, 12].includes(data.type)) {
                    ...
                    return [data.parent_id, true, channelId];
                } else {
                    return [channelId, false, null];
                }
            } catch (err) {
                log.warn(`Could not verify channel ${channelId}, using it anyway:`, err);
                return [channelId, false, null];
            }
        } else {
    ...
    function initUI() {
        ...
        $("button#getChannel").onclick = async () => {
            const [channelId, isThread, threadId] = await getChannelId();
            if (channelId) {
                state.isThread = isThread;
                ...
                log.info(`Channel ID set to: ${channelId}`);
                if (isThread) {
